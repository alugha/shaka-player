...
shaka.abr.SimpleAbrManager = function() {
  ...
  this.preferredWidth_ = Infinity;
  this.preferredHeight_ = Infinity;
};
...
shaka.abr.SimpleAbrManager.prototype.chooseVideoStream_ = function() {
  ...
  var preferredWidth = this.preferredWidth_;
  var preferredHeight = this.preferredHeight_;
  ...
  for (var i = 0; i < videoStreams.length; ++i) {
    ...
    var coversPreferredResolution = stream.width >= preferredWidth && stream.height >= preferredHeight;

    if (currentBandwidth >= minBandwidth && (coversPreferredResolution || currentBandwidth <= maxBandwidth)) {
      if (coversPreferredResolution)
        return stream;
      chosen = stream;
    }
  }
 ...
 *
 * @param {number} width The width in real pixels
 * @param {number} height The height in real pixels
 *   request.
 */
shaka.abr.SimpleAbrManager.prototype.setPreferredResolution = function(width, height) {
