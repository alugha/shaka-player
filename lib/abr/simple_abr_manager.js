...
shaka.abr.SimpleAbrManager.prototype.chooseVideoStream_ = function() {
  ...
  for (var i = 0; i < videoStreams.length; ++i) {
    ...
    var coversPreferredResolution = stream.width >= preferredWidth &&
                                    stream.height >= preferredHeight;

    if (currentBandwidth >= minBandwidth &&
        (coversPreferredResolution || currentBandwidth <= maxBandwidth)) {
 ...
 */
shaka.abr.SimpleAbrManager.prototype.setPreferredResolution = function(
    width, height) {
