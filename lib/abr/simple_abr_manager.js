...
shaka.abr.SimpleAbrManager = function() {
   ...
   * @private {number}
   */
  this.viewportWidth_ = Infinity;

  /**
   * The viewport height in real pixels
   * @private {number}
   */
  this.viewportHeight_ = Infinity;
};
...
/** @override */
shaka.abr.SimpleAbrManager.prototype.setViewport = function(width, height) {
...
shaka.abr.SimpleAbrManager.prototype.chooseVideoStream_ = function() {
  ...
  var viewportWidth = this.viewportWidth_;
  var viewportHeight = this.viewportHeight_;
  var maxWidth = 0;
  videoStreams = videoStreams.filter(function(stream) {
    if (maxWidth && stream.width > maxWidth) {
      return false;
    }
    // Fits viewport?
    if (stream.width >= viewportWidth || stream.height >= viewportHeight) {
